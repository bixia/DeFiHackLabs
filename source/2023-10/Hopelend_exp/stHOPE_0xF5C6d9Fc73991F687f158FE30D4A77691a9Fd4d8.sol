...
library TransferHelper {
    ...
    function doTransferFrom(address tokenAddress, address from, address to, uint256 amount) internal returns(uint256) {
        IERC20_1 token = IERC20_1(tokenAddress);
    ...
    function doTransferIn(
        ...
        IERC20_1 token = IERC20_1(permit.permitted.token);
        ...
        uint256 balanceAfter = IERC20_1(permit.permitted.token).balanceOf(address(this));
        uint256 actualAmount = balanceAfter - balanceBefore;
 ...
 */
contract ERC20 is Context, IERC20_0, IERC20Metadata {
...
contract StakingHOPE is IStaking, ERC20, AbsGauge {
    ...
    function staking(uint256 amount, uint256 nonce, uint256 deadline, bytes memory signature) external override returns (bool) {
        ...
        // checking amount
        uint256 balanceOfUser = IERC20_0(lpToken).balanceOf(staker);
