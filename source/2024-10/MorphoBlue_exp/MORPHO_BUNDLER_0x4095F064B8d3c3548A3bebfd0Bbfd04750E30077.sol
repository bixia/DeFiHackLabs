...
pragma solidity =0.8.24 >=0.5.0 >=0.7.4 >=0.8.0 ^0.8.0 ^0.8.1 ^0.8.17 ^0.8.20 ^0.8.21;
...
// src/ethereum/interfaces/IDaiPermit.sol
...
// lib/public-allocator/lib/metamorpho/lib/openzeppelin-contracts/contracts/token/ERC20/IERC20.sol
...
// lib/public-allocator/lib/metamorpho/lib/openzeppelin-contracts/contracts/token/ERC20/extensions/IERC20Permit.sol
...
type Id_0 is bytes32;

struct MarketParams_0 {
    address loanToken;
...
interface IMorphoBase_0 {
    ...
    function setFee(MarketParams_0 memory marketParams, uint256 newFee) external;
    ...
    function createMarket(MarketParams_0 memory marketParams) external;
    ...
    function supply(
        MarketParams_0 memory marketParams,
        uint256 assets,
        uint256 shares,
        address onBehalf,
        bytes memory data
    ...
    function supplyCollateral(MarketParams_0 memory marketParams, uint256 assets, address onBehalf, bytes memory data)
        external;
    ...
    function withdrawCollateral(MarketParams_0 memory marketParams, uint256 assets, address onBehalf, address receiver)
        external;
    ...
    function setAuthorizationWithSig(Authorization_0 calldata authorization, Signature_0 calldata signature) external;

    /// @notice Accrues interest for the given market `marketParams`.
    function accrueInterest(MarketParams_0 memory marketParams) external;
...
/// @dev Consider using the IMorpho interface instead of this one.
interface IMorphoStaticTyping_0 is IMorphoBase_0 {
    /// @notice The state of the position of `user` on the market corresponding to `id`.
    /// @dev Warning: For `feeRecipient`, `supplyShares` does not contain the accrued shares since the last interest
    /// accrual.
    function position(Id_0 id, address user)
        external
        view
    ...
    function idToMarketParams(Id_0 id)
        external
        view
...
interface IMorpho_0 is IMorphoBase_0 {
    /// @notice The state of the position of `user` on the market corresponding to `id`.
    /// @dev Warning: For `feeRecipient`, `p.supplyShares` does not contain the accrued shares since the last interest
    /// accrual.
    function position(Id_0 id, address user) external view returns (Position_0 memory p);
    ...
    /// interest accrual.
    function market(Id_0 id) external view returns (Market_0 memory m);

    /// @notice The market params corresponding to `id`.
    /// @dev This mapping is not used in Morpho. It is there to enable reducing the cost associated to calldata on layer
    /// 2s by creating a wrapper contract with functions that take `id` as input instead of `marketParams`.
    function idToMarketParams(Id_0 id) external view returns (MarketParams_0 memory);
}

// lib/public-allocator/lib/metamorpho/lib/morpho-blue/src/interfaces/IMorpho.sol

type Id_1 is bytes32;

struct MarketParams_1 {
    address loanToken;
    address collateralToken;
    address oracle;
    address irm;
...
interface IMorphoBase_1 {
    ...
    /// @param newFee The new fee, scaled by WAD.
    /// @dev Warning: The recipient can be the zero address.
    function setFee(MarketParams_1 memory marketParams, uint256 newFee) external;
    ...
    /// @dev The borrow share price of a market with less than 1e4 assets borrowed can be decreased by manipulations, to
    /// the point where `totalBorrowShares` is very large and borrowing overflows.
    function createMarket(MarketParams_1 memory marketParams) external;
    ...
    /// @return sharesSupplied The amount of shares minted.
    function supply(
        MarketParams_1 memory marketParams,
        uint256 assets,
        uint256 shares,
        address onBehalf,
        bytes memory data
    ...
    /// @param onBehalf The address that will own the increased collateral position.
    /// @param data Arbitrary data to pass to the `onMorphoSupplyCollateral` callback. Pass empty data if not needed.
    function supplyCollateral(MarketParams_1 memory marketParams, uint256 assets, address onBehalf, bytes memory data)
        external;
    ...
    /// @param onBehalf The address of the owner of the collateral position.
    /// @param receiver The address that will receive the collateral assets.
    function withdrawCollateral(MarketParams_1 memory marketParams, uint256 assets, address onBehalf, address receiver)
        external;
    ...
    /// @param authorization The `Authorization` struct.
    /// @param signature The signature.
    function setAuthorizationWithSig(Authorization_1 calldata authorization, Signature_1 calldata signature) external;

    /// @notice Accrues interest for the given market `marketParams`.
    function accrueInterest(MarketParams_1 memory marketParams) external;
...
/// @dev This interface is inherited by Morpho so that function signatures are checked by the compiler.
/// @dev Consider using the IMorpho interface instead of this one.
interface IMorphoStaticTyping_1 is IMorphoBase_1 {
    /// @notice The state of the position of `user` on the market corresponding to `id`.
    /// @dev Warning: For `feeRecipient`, `supplyShares` does not contain the accrued shares since the last interest
    /// accrual.
    function position(Id_1 id, address user)
        external
        view
        returns (uint256 supplyShares, uint128 borrowShares, uint128 collateral);
    ...
    /// @dev This mapping is not used in Morpho. It is there to enable reducing the cost associated to calldata on layer
    /// 2s by creating a wrapper contract with functions that take `id` as input instead of `marketParams`.
    function idToMarketParams(Id_1 id)
        external
        view
        returns (address loanToken, address collateralToken, address oracle, address irm, uint256 lltv);
}
...
/// @custom:contact security@morpho.org
/// @dev Use this interface for Morpho to have access to all the functions with the appropriate function signatures.
interface IMorpho_1 is IMorphoBase_1 {
    /// @notice The state of the position of `user` on the market corresponding to `id`.
    /// @dev Warning: For `feeRecipient`, `p.supplyShares` does not contain the accrued shares since the last interest
    /// accrual.
    function position(Id_1 id, address user) external view returns (Position_1 memory p);
    ...
    /// @dev Warning: `m.totalSupplyShares` does not contain the accrued shares by `feeRecipient` since the last
    /// interest accrual.
    function market(Id_1 id) external view returns (Market_1 memory m);

    /// @notice The market params corresponding to `id`.
    /// @dev This mapping is not used in Morpho. It is there to enable reducing the cost associated to calldata on layer
    /// 2s by creating a wrapper contract with functions that take `id` as input instead of `marketParams`.
    function idToMarketParams(Id_1 id) external view returns (MarketParams_1 memory);
}
...
// src/ethereum/libraries/MainnetLib.sol
...
// lib/public-allocator/lib/metamorpho/src/libraries/PendingLib.sol
...
// lib/public-allocator/lib/metamorpho/lib/openzeppelin-contracts/contracts/token/ERC20/extensions/IERC20Metadata.sol
...
// src/interfaces/IMorphoBundler.sol
 ...
 */
interface IERC4626_0 is IERC20_0, IERC20Metadata_0 {
...
// lib/public-allocator/lib/metamorpho/lib/openzeppelin-contracts/contracts/interfaces/IERC4626.sol
 ...
 * https://eips.ethereum.org/EIPS/eip-4626[ERC-4626].
 */
interface IERC4626_1 is IERC20_1, IERC20Metadata_1 {
    event Deposit(address indexed sender, address indexed owner, uint256 assets, uint256 shares);
 ...
 */
contract ERC20_2 is Context, IERC20_0, IERC20Metadata_0 {
...
abstract contract BaseBundler is IMulticall_0 {
    using SafeTransferLib for ERC20_1;
    ...
    function _approveMaxTo(address asset, address spender) internal {
        if (ERC20_1(asset).allowance(address(this), spender) == 0) {
            ERC20_1(asset).safeApprove(spender, type(uint256).max);
        }
    }
}
...
library Permit2Lib {
    ...
    function transferFrom2(ERC20_0 token, address from, address to, uint256 amount) internal {
        // Generate calldata for a standard transferFrom call.
        bytes memory inputData = abi.encodeCall(ERC20_0.transferFrom, (from, to, amount));
    ...
    function permit2(
        ...
        bytes memory inputData = abi.encodeWithSelector(ERC20_0.DOMAIN_SEPARATOR.selector);
        ...
        if (success) {
            ...
            inputData = domainSeparator == DAI_DOMAIN_SEPARATOR
                ...
                : abi.encodeCall(ERC20_0.permit, (owner, spender, amount, deadline, v, r, s));
...
interface IMetaMorphoBase {
    ...
    function MORPHO() external view returns (IMorpho_1);
    ...
    function supplyQueue(uint256) external view returns (Id_1);
    ...
    /// without duplicate.
    function withdrawQueue(uint256) external view returns (Id_1);
    ...
    function submitCap(MarketParams_1 memory marketParams, uint256 newSupplyCap) external;

    /// @notice Accepts the pending cap of the market defined by `marketParams`.
    function acceptCap(MarketParams_1 memory marketParams) external;

    /// @notice Revokes the pending cap of the market defined by `id`.
    /// @dev Does not revert if there is no pending cap.
    function revokePendingCap(Id_1 id) external;
    ...
    function submitMarketRemoval(MarketParams_1 memory marketParams) external;

    /// @notice Revokes the pending removal of the market defined by `id`.
    /// @dev Does not revert if there is no pending market removal.
    function revokePendingMarketRemoval(Id_1 id) external;
    ...
    function setSupplyQueue(Id_1[] calldata newSupplyQueue) external;
...
interface IMetaMorphoStaticTyping is IMetaMorphoBase {
    ...
    function config(Id_1) external view returns (uint184 cap, bool enabled, uint64 removableAt);

    /// @notice Returns the pending guardian.
    function pendingGuardian() external view returns (address guardian, uint64 validAt);

    /// @notice Returns the pending cap for each market.
    function pendingCap(Id_1) external view returns (uint192 value, uint64 validAt);
...
interface IMetaMorpho is IMetaMorphoBase, IERC4626_1, IERC20Permit_1, IOwnable, IMulticall_1 {
    /// @notice Returns the current configuration of each market.
    function config(Id_1) external view returns (MarketConfig memory);

    /// @notice Returns the pending guardian.
    function pendingGuardian() external view returns (PendingAddress memory);

    /// @notice Returns the pending cap for each market.
    function pendingCap(Id_1) external view returns (PendingUint192 memory);
 ...
 */
abstract contract ERC20Wrapper is ERC20_2 {
    IERC20_0 private immutable _underlying;

    constructor(IERC20_0 underlyingToken) {
     ...
     */
    function decimals() public view virtual override returns (uint8) {
        try IERC20Metadata_0(address(_underlying)).decimals() returns (uint8 value) {
            return value;
        } catch {
     ...
     */
    function underlying() public view returns (IERC20_0) {
        return _underlying;
    }
...
// lib/public-allocator/src/interfaces/IPublicAllocator.sol
...
interface IPublicAllocatorBase {
    ...
    function reallocateTo(address vault, Withdrawal_0[] calldata withdrawals, MarketParams_1 calldata supplyMarketParams)
        external
        payable;
    ...
    function setFlowCaps(address vault, FlowCapsConfig_0[] calldata config) external;
}
...
interface IPublicAllocatorStaticTyping is IPublicAllocatorBase {
    ...
    function flowCaps(address vault, Id_1) external view returns (uint128, uint128);
}
...
/// signatures.
interface IPublicAllocator_0 is IPublicAllocatorBase {
    /// @notice Returns the maximum inflow and maximum outflow through public allocation of a given market for a given
    /// vault.
    function flowCaps(address vault, Id_1) external view returns (FlowCaps_0 memory);
}

// src/PermitBundler.sol
...
abstract contract PermitBundler is BaseBundler {
    ...
    function permit(address asset, uint256 amount, uint256 deadline, uint8 v, bytes32 r, bytes32 s, bool skipRevert)
        ...
        protected
    {
        try IERC20Permit_0(asset).permit(initiator(), address(this), amount, deadline, v, r, s) {}
...
abstract contract TransferBundler is BaseBundler {
    ...
    function erc20Transfer(address asset, address recipient, uint256 amount) external payable protected {
        ...
        amount = Math.min(amount, ERC20_1(asset).balanceOf(address(this)));

        if (amount == 0) return;

        ERC20_1(asset).safeTransfer(recipient, amount);
    }
    ...
    function erc20TransferFrom(address asset, uint256 amount) external payable protected {
        ...
        amount = Math.min(amount, ERC20_1(asset).balanceOf(_initiator));

        require(amount != 0, ErrorsLib.ZERO_AMOUNT);

        ERC20_1(asset).safeTransferFrom(_initiator, address(this), amount);
    }
}
...
abstract contract WNativeBundler is BaseBundler {
    ...
    function unwrapNative(uint256 amount) external payable protected {
        amount = Math.min(amount, ERC20_1(WRAPPED_NATIVE).balanceOf(address(this)));
...
struct FlowCapsConfig_1 {
    Id_0 id;
    FlowCaps_1 caps;
}

struct Withdrawal_1 {
    MarketParams_0 marketParams;
    uint128 amount;
}

/// @dev Copy of the Public Allocator interface but using the struct of morpho-blue imported by the bundler.
interface IPublicAllocator_1 {
    function MORPHO() external view returns (IMorpho_0);
    function owner(address vault) external view returns (address);
    function fee(address vault) external view returns (uint256);
    function accruedFee(address vault) external view returns (uint256);
    function reallocateTo(address vault, Withdrawal_1[] calldata withdrawals, MarketParams_0 calldata supplyMarketParams)
        external
        payable;
    function setOwner(address vault, address newOwner) external;
    function setFee(address vault, uint256 newFee) external;
    function transferFee(address vault, address payable feeRecipient) external;
    function setFlowCaps(address vault, FlowCapsConfig_1[] calldata config) external;
    function flowCaps(address vault, Id_0) external view returns (FlowCaps_1 memory);
}

// src/StEthBundler.sol
...
abstract contract StEthBundler is BaseBundler {
    ...
    function wrapStEth(uint256 amount) external payable protected {
        amount = Math.min(amount, ERC20_1(ST_ETH).balanceOf(address(this)));
    ...
    function unwrapStEth(uint256 amount) external payable protected {
        amount = Math.min(amount, ERC20_1(WST_ETH).balanceOf(address(this)));
...
abstract contract ERC4626Bundler is BaseBundler {
        ...
        protected
    {
        ...
        _approveMaxTo(IERC4626_0(vault).asset(), vault);

        uint256 assets = IERC4626_0(vault).mint(shares, receiver);
        ...
        protected
    {
        ...
        address asset = IERC4626_0(vault).asset();
        assets = Math.min(assets, ERC20_1(asset).balanceOf(address(this)));

        require(assets != 0, ErrorsLib.ZERO_AMOUNT);

        _approveMaxTo(asset, vault);

        uint256 shares = IERC4626_0(vault).deposit(assets, receiver);
        ...
        protected
    {
        ...
        uint256 shares = IERC4626_0(vault).withdraw(assets, receiver, owner);
        ...
        protected
    {
        ...
        shares = Math.min(shares, IERC4626_0(vault).balanceOf(owner));

        require(shares != 0, ErrorsLib.ZERO_SHARES);

        uint256 assets = IERC4626_0(vault).redeem(shares, receiver, owner);
        require(assets * initialShares >= minAssets * shares, ErrorsLib.SLIPPAGE_EXCEEDED);
    }
}

// src/ethereum/EthereumPermitBundler.sol
...
// src/ethereum/EthereumStEthBundler.sol
...
abstract contract ERC20WrapperBundler is BaseBundler {
    ...
    function erc20WrapperDepositFor(address wrapper, uint256 amount) external payable protected {
        ERC20_1 underlying = ERC20_1(address(ERC20Wrapper(wrapper).underlying()));
    ...
    function erc20WrapperWithdrawTo(address wrapper, address account, uint256 amount) external payable protected {
        ...
        amount = Math.min(amount, ERC20_1(wrapper).balanceOf(address(this)));
...
abstract contract MorphoBundler is BaseBundler, IMorphoBundler {
    ...
    IMorpho_0 public immutable MORPHO;
    ...
    function morphoSetAuthorizationWithSig(
        Authorization_0 calldata authorization,
        Signature_0 calldata signature,
        bool skipRevert
    ) external payable protected {
    ...
    function morphoSupply(
        MarketParams_0 calldata marketParams,
        uint256 assets,
        uint256 shares,
        ...
        if (assets == type(uint256).max) assets = ERC20_1(marketParams.loanToken).balanceOf(address(this));
    ...
    function morphoSupplyCollateral(
        ...
        if (assets == type(uint256).max) assets = ERC20_1(marketParams.collateralToken).balanceOf(address(this));
    ...
    /// @param assets The amount of collateral to withdraw.
    /// @param receiver The address that will receive the collateral assets.
    function morphoWithdrawCollateral(MarketParams_0 calldata marketParams, uint256 assets, address receiver)
        external
        payable
        protected
    {
    ...
    function reallocateTo(
        ...
        address vault,
        uint256 value,
        Withdrawal_1[] calldata withdrawals,
        MarketParams_0 calldata supplyMarketParams
    ) external payable protected {
        IPublicAllocator_1(publicAllocator).reallocateTo{value: value}(vault, withdrawals, supplyMarketParams);
    }
