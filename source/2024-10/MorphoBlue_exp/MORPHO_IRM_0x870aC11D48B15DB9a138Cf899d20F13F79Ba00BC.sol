...
pragma solidity =0.8.19 >=0.5.0 ^0.8.0;
...
// src/AdaptiveCurveIrm.sol

/// @title AdaptiveCurveIrm
/// @author Morpho Labs
/// @custom:contact security@morpho.org
contract AdaptiveCurveIrm is IAdaptiveCurveIrm {
    using MathLib_1 for int256;
    using UtilsLib for int256;
    using MathLib_0 for uint128;
    ...
    function _curve(int256 _rateAtTarget, int256 err) private pure returns (int256) {
        ...
        int256 coeff = err < 0 ? WAD_INT - WAD_INT.wDivToZero(ConstantsLib.CURVE_STEEPNESS) : ConstantsLib.CURVE_STEEPNESS - WAD_INT;
        // Non negative if _rateAtTarget >= 0 because if err < 0, coeff <= 1.
        return (coeff.wMulToZero(err) + WAD_INT).wMulToZero(int256(_rateAtTarget));
    }
