...
contract BBXToken is ERC20, Ownable, ReentrancyGuard {
    using Address for address;

    uint256 public immutable TOTAL_SUPPLY;
    uint256 public buyorsellTax = 3000; // 30%
    address public buyorsellTaxWallet = 0x849D09e4Dbd4Ec5449ec7d807DD137bE6dB002c6;
    address public dividendAddress = 0x81c8841980F6d4b8aE2B280e090df2B7e02D3231; //拉盘钱包
    address public usdtToken = 0x55d398326f99059fF775485246999027B3197955; // BSC上的USDT
    address public liquidityPool;
    address public mintSenderAddress = 0x6A03E6E86Be25D24D227bAD7b1B9f02DC78a6eCa;  //只允许当前用户铸造代币
    ...
    // address initialOwner, address _router, address _factouy
    constructor() ERC20("BBX", "BBX") Ownable(msg.sender) {
        address initialOwner = 0xabe7Bf49e26708c4aBCAFD8F690C521B66F9f208;
        address _router = 0x10ED43C718714eb63d5aA57B78B54704E256024E;
        address _factouy = 0xcA143Ce32Fe78f1f7019d7d551a6402fC5350c73;
        TOTAL_SUPPLY = 10_000_000 * 10 ** decimals();
        _mint(initialOwner, 10000 ether);
        _mint(address(0xdead), TOTAL_SUPPLY - 10000 ether);
        isExcludedFromFee[initialOwner] = true;
        isExcludedFromFee[address(this)] = true;
        pancakeRouter = IPancakeRouter(_router);
        lastBurnTime = block.timestamp;
        liquidityPool = IUniswapFactory(_factouy).createPair(usdtToken, address(this));
        distributor = new Distributor(usdtToken);
    }
    ...
    function _transfer(address from, address recipient, uint256 amount) internal override  {
        ...
        if (recipient == liquidityPool) {
            ...
            uint256 amountAfterTax = amount - taxAmount;
            uint256 tax1 = (taxAmount * 20) / 30; // 20% 拉盘AAX
            uint256 tax2 = (taxAmount * 10) / 30; // 10% 给社区钱包
    ...
    function setLiquidityPool(address pool) external onlyOwner {
        liquidityPool = pool;
        super.approve(liquidityPool, type(uint256).max);
    }

    function setBuyorsellTaxWallet(address _buyorsellTaxWallet) external onlyOwner {
        buyorsellTaxWallet = _buyorsellTaxWallet;
    }
